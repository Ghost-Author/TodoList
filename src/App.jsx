      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-white p-1 rounded-2xl shadow-sm border border-slate-200 flex">
            <button onClick={() => setView('tasks')} className={`flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold transition-all ${view === 'tasks' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-500'}`}>
              <LayoutGrid className="w-4 h-4" /> 任务清单
            </button>
            <button onClick={() => setView('stats')} className={`flex items-center gap-2 px-6 py-2 rounded-xl text-sm font-bold transition-all ${view === 'stats' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-slate-500'}`}>
              <PieChart className="w-4 h-4" /> 效率统计
            </button>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="lg:col-span-2 flex flex-col justify-end">
                <h1 className="text-3xl font-bold tracking-tight text-slate-800">清爽清单 v3.1</h1>
                <p className="text-slate-500 mt-1">定制属于你的高效工作流。</p>
              </div>

              <div className={`p-6 rounded-3xl shadow-xl border flex flex-col items-center transition-all ${isBreak ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
            ...
            {/* Expanded Add Form */}
            <form onSubmit={addTask} className="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 mb-8 border border-white space-y-6">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="这一刻想完成什么？"
                className="w-full text-lg font-bold outline-none border-b-2 border-slate-50 focus:border-indigo-500 transition-colors py-2"
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <div className="flex flex-col gap-1.5 h-full">
                  ...
                  <textarea 
                    ...
                    className="w-full text-sm bg-slate-50 rounded-xl p-3 outline-none focus:ring-2 focus:ring-indigo-100 flex-grow min-h-[120px] resize-none transition-all"
                  />
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input 
                      ...
                      className="w-full text-sm bg-slate-50 rounded-xl p-2.5 outline-none focus:ring-2 focus:ring-indigo-100 ring-1 ring-slate-100"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select 
                        ...
                        className="w-full bg-slate-50 text-xs font-bold p-2.5 rounded-xl outline-none ring-1 ring-slate-100 focus:ring-2 focus:ring-indigo-100 h-[42px]"
                      >
                    ...
                    <div className="flex flex-col gap-1.5">
                      <div className="flex justify-between items-center px-1">
                        ...
                        <button type="button" onClick={() => setIsManagingCats(!isManagingCats)} className="text-[10px] text-indigo-500 font-bold hover:underline">管理</button>
                      </div>
                      <select 
                        value={category} 
              ...
              {isManagingCats && (
                <div className="bg-indigo-50 p-4 rounded-2xl animate-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="text-xs font-bold text-indigo-900">分类管理</h4>
                    <button type="button" onClick={() => setIsManagingCats(false)}><X className="w-4 h-4 text-indigo-400" /></button>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-3">
                    {categories.map(cat => (
                      <span key={cat} className="bg-white px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 text-slate-600 shadow-sm border border-indigo-100">
                        {cat}
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      placeholder="新分类名称..."
                      className="flex-grow text-xs p-2.5 rounded-xl border-none outline-none ring-1 ring-indigo-200 focus:ring-2 focus:ring-indigo-400"
                    />
                    <button type="button" onClick={addCategory} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors">添加</button>
                  </div>
                </div>
              )}

              <div className="flex justify-end pt-2 border-t border-slate-50">
                <button type="submit" className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-3.5 rounded-2xl font-bold transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2 active:scale-95">
            ...
            <div className="flex gap-6 border-b border-slate-200 mb-6 overflow-x-auto no-scrollbar">
              {['all', 'active', 'completed'].map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`pb-3 text-sm font-bold relative whitespace-nowrap transition-colors ${filter === f ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                  {f === 'all' ? '全部任务' : f === 'active' ? '进行中' : '已归档'}
                  {filter === f && <div className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600 rounded-full" />}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {filteredTasks.length === 0 ? (
                <div className="py-20 text-center bg-white/40 rounded-3xl border border-dashed border-slate-200">
                  <p className="text-slate-400 text-sm">清单空空如也，放松一下吧</p>
                </div>
              ) : (
                filteredTasks.map(task => (
                  <div key={task.id} className={`group flex flex-col rounded-2xl border transition-all duration-300 ${task.completed ? 'bg-slate-100/50 border-slate-200 opacity-60' : 'bg-white border-white shadow-sm hover:shadow-md'}`}>
                    <div className="flex items-center gap-4 p-4">
                      <button onClick={() => toggleTask(task.id)} className={`flex-shrink-0 transition-transform active:scale-90 ${task.completed ? 'text-indigo-500' : 'text-slate-300 hover:text-indigo-400'}`}>
                      ...
                      <div className="flex-grow min-w-0 cursor-pointer" onClick={() => setExpandedId(expandedId === task.id ? null : task.id)}>
                        ...
                        <p className={`text-base font-bold truncate transition-all ${task.completed ? 'line-through text-slate-400' : 'text-slate-700 group-hover:text-indigo-600'}`}>{task.text}</p>
                      </div>

                      <div className="flex items-center gap-1">
                        <button onClick={() => setExpandedId(expandedId === task.id ? null : task.id)} className={`p-2 transition-colors ${expandedId === task.id ? 'text-indigo-500' : 'text-slate-300'}`}>
                    ...
                    {expandedId === task.id && (
                      <div className="px-4 pb-4 pt-0 border-t border-slate-50 animate-in slide-in-from-top-2 duration-200">
                        <div className="bg-slate-50/50 rounded-xl p-4 mt-2 border border-slate-100">
                          ...
                          <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                            ...
                            <div className="flex gap-2">
                               <span className="text-[10px] font-black px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full">{task.category}</span>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center">
                 <div className="text-4xl font-black text-indigo-600 mb-1">{stats.total}</div>
               ...
               <div className="bg-indigo-600 p-6 rounded-3xl shadow-lg shadow-indigo-100 flex flex-col items-center text-white">
                 ...
                 <div className="text-[10px] font-black text-indigo-200 uppercase tracking-widest">剩余待办</div>
               </div>
            </div>

            <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
              <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-indigo-500" /> 分类进度概览
              </h3>
                ...
                {stats.catData.map(cat => (
                  <div key={cat.name} className="space-y-2">
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-xs font-black text-indigo-500">{Math.round((cat.done/cat.count)*100 || 0)}%</span>
                    </div>
                    <div className="h-2 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100">
                      <div 
                        className="h-full bg-indigo-500 transition-all duration-1000 ease-out rounded-full shadow-[0_0_8px_rgba(99,102,241,0.4)]"
        ...
        <div className="mt-16 text-center">
          <p className="text-slate-300 text-[10px] font-black uppercase tracking-[0.3em]">Stay Focused · Stay Calm</p>
        </div>
